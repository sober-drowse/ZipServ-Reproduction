...
第一个问题是服务器访问 GitHub HTTPS 不稳定，所以我改用 SSH 克隆和推送。

第二个问题是官方 Makefile 默认找 `/usr/local/cuda/bin/nvcc`，但服务器实际是 `/usr/bin/nvcc`。我在脚本里显式传入 `CUDA_INSTALL_PATH=/usr CUDA_HOME=/usr`。

第三个问题是 CUDA 12.0 下 BF16 intrinsic 的 host/device 调用不兼容。官方代码在 host 函数里调用了部分 device 函数，会编译失败。我的解决办法是增加 host 端可用的 BF16 bit 转换函数，用 `std::memcpy` 保留 bit 表示。
